...
// request the parameters that start the simulation
	...
	//starting measure = 0.000000, minimum = 0, equilibrating (10000 moves)...
	double dE;
	int m;
	
	double S = 0.0;
	
	Uold = penalty(p, upd, measureold);//add U* to starting configuration
	...
	int i = 0;
	int j = 0;
	int pstruc=0; //counting structures to be print
	...
	while (1) {
		...
		if (accept(beta*(dE + S + Unew - Uold))) {
			...
			/*
			if (pstruc<10){//bin(p, measurenew)==0 && 
				//char struc[1];
				//char struc = (char)(((int)'0')+pstruc);
				char stepfile[100];
				char pdbfile[100];
				//itoa(pstruc,struc,10);
				//char directory[]="/home/juanwei/mycode/dnatools/output/stru-";
				printf("0\n");
				sprintf(stepfile, "/home/juanwei/mycode/dnatools/output/structure-ID%d.dat", pstruc);
				printf("1\n");
				sprintf(pdbfile, "/home/juanwei/mycode/dnatools/output/proteins-ID%d.pdb", pstruc);	
				printf("2\n");
				//strcpy(stepfile,directory); strcat(stepfile,struc); strcat(stepfile,".dat");
				//strcpy(pdbfile,directory); strcat(pdbfile,struc); strcat(pdbfile,".pdb");
				b->illustrate_config(stepfile,pdbfile);
				printf("3\n");
				pstruc++;
			}
			*/
