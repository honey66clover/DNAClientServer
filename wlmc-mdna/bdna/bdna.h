...
public:
  ...
  matrix I[number_of_bases*number_of_bases]; //6*1 step parameters in equilibrium 
  matrix F[number_of_bases*number_of_bases]; //6*6 force constants
  matrix *v;
  matrix *phosphate;
  ...
  FLOAT DNA_esenergy(matrix &W, matrix *V);
  FLOAT calculate_penergy();

  FLOAT calculate_link3();
  FLOAT calculate_link();
  FLOAT calculate_link_new();
  FLOAT calculate_link_open();
  FLOAT calculate_writhe_open();
