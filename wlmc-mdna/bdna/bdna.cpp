...
//  for (int i = 0; i < nsteps; i++) v[i].setsize(6,1);  
  ...
  readFparameters("/home/juanwei/mycode/dnatools/wlmc-mdna/FGH.dat", F);
  readIparameters("/home/juanwei/mycode/dnatools/wlmc-mdna/tp0.dat", I);
...
// electrostatic (dampened by ion screening) energy and retrun end to end matrix W
FLOAT bdna::DNA_esenergy(matrix &W, matrix *V) {//can pass v or vchange to V for old and new configurations
...
FLOAT bdna::calculate_link_open() {
...
FLOAT bdna::calculate_writhe_open() {
  FLOAT taul = 0.0,  
    wrest = 0.0,
    wrl;
  FLOAT tau;
...
void bdna::print3dna(char *filename) {
  ...
  for (int i = 0; i < nsteps; i++) {
	matrix vi = v[i];
    fprintf(f, "A-T   %6.2lf %6.2lf %6.2lf %6.2lf %6.2lf %6.2lf\n", vi(4,1), vi(5,1), vi(6,1), vi(1,1), vi(2,1), vi(3,1));
  }
  fclose(f);
}
